 ...
 * @LastEditTime: 2024-01-12 21:52:20
 */
...
template <typename conf = serialize_props_default, typename Writer,
...
template <typename conf = serialize_props_default,
          typename Buffer = std::vector<char>, typename... Args>
...
template <typename conf = serialize_props_default, typename T, typename... Args,
          typename View,
...
template <typename conf, typename... Args, typename View,
          std::enable_if_t<
              detail::deserialize_view<View> && detail::props_t<conf>, int> = 0>
...
template <
    typename conf, typename... Args, typename View,
    std::enable_if_t<detail::deserialize_view<View> && !detail::props_t<conf>,
                     int> = 0>
auto deserialize(const View &v) {
  expected<detail::get_args_type<conf, Args...>, serialize::return_code> ret;
...
template <typename conf, typename... Args, typename Reader,
          std::enable_if_t<detail::props_t<conf>, int> = 0>
