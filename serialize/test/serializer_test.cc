...
TEST(SerializerTest, SerializeTest) {
  ...
  EXPECT_EQ(serialize::serialize(p2).size(), 21);
  person3 p3{{2, 2.2, 2}, {1, 1.2, {1, 2}}};
  EXPECT_EQ(serialize::serialize(p3).size(), 37);
  ...
  //     std::cout << (int)b << " ";
  //   }
  //   std::cout << buffer_vec_int.size() << "---cc-- " << ss.str() <<
  //   std::endl; ss.clear(); std::vector<std::string> str_vec{"a", "bb",
  //   "cc"}; auto buffer_vec_str = serialize::serialize(str_vec); for (auto b
  //   : buffer_vec_str) {
  //     ss << b;
  //   }
  //   std::cout << "---cc-- " << std::endl;
  ...
  //   std::cout << buffer_vec_str.size() << "---cc-- " << ss.str() <<
  //   std::endl; std::pair<int, double> pair1{1, 2.0}; auto buffer_pair1 =
  //   serialize::serialize(pair1); std::cout << buffer_pair1.size() <<
  //   "---cc-- " << std::endl;
}