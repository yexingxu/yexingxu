...
struct if_constexpr {
  ...
  template <
      bool check = result,
      std::enable_if_t<!check && !std::is_same<FalseT, std::nullptr_t>::value,
                       int> = 0>
  constexpr auto operator()() {
    return false_();
  }
...
struct else_ {
  ThenT then_;
  constexpr explicit else_(ThenT then) : then_{std::move(then)} {}
};
...
template <bool result, typename TrueT, typename ElseT,
          std::enable_if_t<
              detail::is_specialization<ElseT, detail::else_>::value, int> = 0>
...
constexpr auto default_(DefaultT &&default_) {
  return detail::case_constexpr<true, DefaultT>{
      std::forward<DefaultT>(default_)};
}

template <typename LastT, std::enable_if_t<LastT::value, int> = 0>
constexpr auto switch_(LastT &&last) {
  return last();
}

template <typename LastT, std::enable_if_t<!LastT::value, int> = 0>
constexpr auto switch_(LastT &&) {}

template <typename FirstT, typename... CasesT,
          std::enable_if_t<FirstT::value, int> = 0>
constexpr auto switch_(FirstT &&first, CasesT &&...) {
  return first();
}

template <typename FirstT, typename... CasesT,
          std::enable_if_t<!FirstT::value, int> = 0>
constexpr auto switch_(FirstT &&, CasesT &&...cases) {
