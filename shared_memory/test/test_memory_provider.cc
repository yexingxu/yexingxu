...
class MemoryProviderFailingCreation : public MemoryProvider {
 public:
  using MemoryProvider::getErrorString;

  tl::expected<void*, MemoryProviderError> createMemory(
      const uint64_t size [[maybe_unused]],
  ...
  tl::expected<void, MemoryProviderError> destroyMemory() noexcept override {
    return tl::make_unexpected(MemoryProviderError::MEMORY_DESTRUCTION_FAILED);
  }
};
...
class MemoryProvider_Test : public Test {
 public:
  void SetUp() override {
    // since the MemoryProvider registers for relative pointer, it is necessary
    // to call unregisterAll, to have a clean environment especially for the
    // first test
    UntypedRelativePointer::unregisterAll();
  }

  void TearDown() override {
    // unregisterAll is also called to leave a clean environment after the last
    // test
    UntypedRelativePointer::unregisterAll();
  }
  ...
  tl::expected<void, MemoryProviderError> commonSetup() {
    EXPECT_FALSE(!sut.addMemoryBlock(&memoryBlock1).has_value());
    EXPECT_CALL(memoryBlock1, size())
        .WillRepeatedly(Return(COMMON_SETUP_MEMORY_SIZE));
    EXPECT_CALL(memoryBlock1, alignment())
        .WillRepeatedly(Return(COMMON_SETUP_MEMORY_ALIGNMENT));
    EXPECT_CALL(sut, createMemoryMock(COMMON_SETUP_MEMORY_SIZE,
  ...
  static constexpr MemoryProviderError m_testCombinationMemoryProviderError[] =
      {MemoryProviderError::MEMORY_BLOCKS_EXHAUSTED,
       MemoryProviderError::NO_MEMORY_BLOCKS_PRESENT,
       MemoryProviderError::MEMORY_ALREADY_CREATED,
       MemoryProviderError::MEMORY_CREATION_FAILED,
       MemoryProviderError::MEMORY_ALIGNMENT_EXCEEDS_PAGE_SIZE,
       MemoryProviderError::MEMORY_ALLOCATION_FAILED,
       MemoryProviderError::MEMORY_MAPPING_FAILED,
       MemoryProviderError::MEMORY_NOT_AVAILABLE,
       MemoryProviderError::MEMORY_DESTRUCTION_FAILED,
       MemoryProviderError::MEMORY_DEALLOCATION_FAILED,
       MemoryProviderError::MEMORY_UNMAPPING_FAILED,
       MemoryProviderError::SIGACTION_CALL_FAILED};

  static constexpr const char* m_testResultGetErrorString[] = {
  ...
  // the MemoryProvider is a class with a pure virtual member functions s,
  // therefore we need an implementation to instantiate and test non-virtual
  // member functions
  MemoryProviderMock sut;
};

constexpr MemoryProviderError
    MemoryProvider_Test::m_testCombinationMemoryProviderError[];
...
TEST_F(MemoryProvider_Test, AddMemoryBlockExceedsCapacity) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "5503e89e-d927-4669-a0ec-1fa048df373e");
  MemoryBlockMock
      memoryBlocks[MAX_NUMBER_OF_MEMORY_BLOCKS_PER_MEMORY_PROVIDER + 1];

  for (uint32_t i = 0; i < MAX_NUMBER_OF_MEMORY_BLOCKS_PER_MEMORY_PROVIDER;
       ++i) {
    EXPECT_THAT(!sut.addMemoryBlock(&memoryBlocks[i]).has_value(), Eq(false));
  }

  auto expectError = sut.addMemoryBlock(
      &memoryBlocks[MAX_NUMBER_OF_MEMORY_BLOCKS_PER_MEMORY_PROVIDER]);
  ASSERT_THAT(!expectError.has_value(), Eq(true));
  EXPECT_THAT(expectError.error(),
              Eq(MemoryProviderError::MEMORY_BLOCKS_EXHAUSTED));
}

TEST_F(MemoryProvider_Test, CreateWithoutMemoryBlock) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "82f4bcac-3d44-4152-8d6a-ad72cb4ec834");
  EXPECT_CALL(sut, createMemoryMock(_, _)).Times(0);
  auto expectError = sut.create();
  ASSERT_THAT(!expectError.has_value(), Eq(true));
  EXPECT_THAT(expectError.error(),
              Eq(MemoryProviderError::NO_MEMORY_BLOCKS_PRESENT));
...
TEST_F(MemoryProvider_Test, CreateWithCommonSetupOfOneMemoryBlockIsSuccessful) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "0d4a3cba-35c2-4787-b1aa-7c5325fe505c");
  auto expectSuccess = commonSetup();

  EXPECT_THAT(!expectSuccess.has_value(), Eq(false));
...
TEST_F(MemoryProvider_Test, CreationFailed) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "b47cd296-8fb2-4ae7-ad80-9c962eff687f");
  MemoryProviderFailingCreation sutFailure;
  ASSERT_FALSE(!sutFailure.addMemoryBlock(&memoryBlock1).has_value());
  ...
  ASSERT_THAT(!expectError.has_value(), Eq(true));
  EXPECT_THAT(expectError.error(),
              Eq(MemoryProviderError::MEMORY_CREATION_FAILED));
...
TEST_F(MemoryProvider_Test, CreateAndAnnounceWithOneMemoryBlock) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "a090b31e-7bb9-4461-b644-2ae0384824f6");
  ASSERT_FALSE(!commonSetup().has_value());
...
TEST_F(MemoryProvider_Test, CreateAndAnnounceWithMultipleMemoryBlocks) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "e4c13cc4-6596-4902-be7b-99b801a89cc0");
  ASSERT_FALSE(!sut.addMemoryBlock(&memoryBlock1).has_value());
  ASSERT_FALSE(!sut.addMemoryBlock(&memoryBlock2).has_value());
  ...
  EXPECT_CALL(memoryBlock1, alignment())
      .WillRepeatedly(Return(MEMORY_ALIGNMENT_1));
  EXPECT_CALL(memoryBlock2, size()).WillRepeatedly(Return(MEMORY_SIZE_2));
  EXPECT_CALL(memoryBlock2, alignment())
      .WillRepeatedly(Return(MEMORY_ALIGNMENT_2));
  EXPECT_CALL(
      sut, createMemoryMock(MEMORY_SIZE_1 + MEMORY_SIZE_2,
                            std::max(MEMORY_ALIGNMENT_1, MEMORY_ALIGNMENT_2)))
      .Times(1);
...
TEST_F(MemoryProvider_Test, AddMemoryBlockAfterCreation) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "04e8514a-9ea5-4027-8415-7aaf1ffc5637");
  ASSERT_FALSE(!commonSetup().has_value());

  auto expectError = sut.addMemoryBlock(&memoryBlock2);
  ASSERT_THAT(!expectError.has_value(), Eq(true));
  EXPECT_THAT(expectError.error(),
              Eq(MemoryProviderError::MEMORY_ALREADY_CREATED));
}

TEST_F(MemoryProvider_Test, MultipleCreates) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "6e1c1168-da0c-4c20-b027-16b641683f30");
  ASSERT_FALSE(!commonSetup().has_value());
...
TEST_F(MemoryProvider_Test, MultipleAnnouncesAreSuppressed) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "cfc04605-ad22-4e97-b587-0dd13db63765");
  ASSERT_FALSE(!commonSetup().has_value());

  EXPECT_CALL(memoryBlock1, onMemoryAvailable(_)).Times(1);
  sut.announceMemoryAvailable();
  sut.announceMemoryAvailable();  // this shouldn't trigger a second
                                  // memoryAvailable call on memoryBlock1
...
TEST_F(MemoryProvider_Test, MultipleDestroys) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "61f21297-511f-4c09-b560-c6e2a93cb20e");
  ASSERT_FALSE(!commonSetup().has_value());

  EXPECT_THAT(!sut.destroy().has_value(), Eq(false));

  auto expectError = sut.destroy();
  ASSERT_THAT(!expectError.has_value(), Eq(true));
  EXPECT_THAT(expectError.error(),
              Eq(MemoryProviderError::MEMORY_NOT_AVAILABLE));
}

TEST_F(MemoryProvider_Test, IntialBaseAddressValueIsUnset) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "0de67825-644e-49ea-9cbb-48cd22855260");
  EXPECT_THAT(sut.baseAddress().has_value(), Eq(false));
}

TEST_F(MemoryProvider_Test, BaseAddressValueAfterCreationIsValid) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "09e1ded2-658c-41fd-a9c1-7a257d30af2e");
  ASSERT_FALSE(!commonSetup().has_value());
...
TEST_F(MemoryProvider_Test, BaseAddressValueAfterDestructionIsUnset) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "22c77eeb-5c27-4690-915e-bf9cd004ff89");
  ASSERT_FALSE(!commonSetup().has_value());
...
TEST_F(MemoryProvider_Test, SizeValueAfterDestructionIsZero) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "28ef9db3-310f-46ab-88b6-253a1a56eb26");
  ASSERT_FALSE(!commonSetup().has_value());
...
TEST_F(MemoryProvider_Test, SegmentIdValueAfterCreationIsValid) {
  ...
  // the segment id being monotonic increasing is an implementation detail, in
  // the case that the implementation changes, just remove this check, since we
  // already check to get a valid result
  EXPECT_THAT(segmentId.value(), Eq(segmentIdOffset.value() + 1U));
}

TEST_F(MemoryProvider_Test, SegmentIdValueAfterDestructionIsUnset) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "c011594c-1a56-4857-ad23-65e91c5b99fd");
  ASSERT_FALSE(!commonSetup().has_value());
...
TEST_F(MemoryProvider_Test, GetErrorString) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "68b8d3b6-0d70-4aac-9c92-19f9f27a86d7");
  constexpr int32_t NUMBER_OF_TEST_CASES =
      sizeof(m_testCombinationMemoryProviderError) /
      sizeof(MemoryProviderError);

  for (int16_t i = 0; i < NUMBER_OF_TEST_CASES; i++) {
    const char* result = MemoryProviderFailingCreation::getErrorString(
        m_testCombinationMemoryProviderError[i]);
