...
TEST(ChunkSettings_test, CallingUserPayloadSizeReturnsCorrectValue) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "09b4d176-7fab-4957-8eb7-9c274dfa0ab1");
  ...
  auto sutResult =
      ChunkSettings::create(USER_PAYLOAD_SIZE, USER_PAYLOAD_ALIGNMENT,
                            USER_HEADER_SIZE, USER_HEADER_ALIGNMENT);
...
TEST(ChunkSettings_test, CallingUserPayloadAlignmentReturnsCorrectValue) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "310e7342-9654-4c27-8be8-419b63f5c783");
  constexpr uint32_t USER_PAYLOAD_SIZE{42U};
  constexpr uint32_t USER_PAYLOAD_ALIGNMENT{128U};
  constexpr uint32_t USER_HEADER_SIZE{64U};
  constexpr uint32_t USER_HEADER_ALIGNMENT{4U};
...
TEST(ChunkSettings_test, CallingUserHeaderSizeReturnsCorrectValue) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "c39d8d59-1f37-428c-b902-16e8eb8f80e7");
  constexpr uint32_t USER_PAYLOAD_SIZE{42U};
  constexpr uint32_t USER_PAYLOAD_ALIGNMENT{128U};
  constexpr uint32_t USER_HEADER_SIZE{64U};
  constexpr uint32_t USER_HEADER_ALIGNMENT{4U};
...
TEST(ChunkSettings_test, CallingUserHeaderAlignmentReturnsCorrectValue) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "cf75ffae-7ace-4eb7-b6ab-c6dfc7e3251e");
  constexpr uint32_t USER_PAYLOAD_SIZE{42U};
  constexpr uint32_t USER_PAYLOAD_ALIGNMENT{128U};
  constexpr uint32_t USER_HEADER_SIZE{64U};
  constexpr uint32_t USER_HEADER_ALIGNMENT{4U};
...
TEST(ChunkSettings_test, CallingRequiredChunkSizeReturnsCorrectValue) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "8e8ba2a5-467b-4bf3-9231-2228187c9ca7");
...
TEST(ChunkSettings_test,
     NoCustomUserPayloadAlignmentAndTooLargeUserPayload_Fails) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "1ac315d5-fb8d-4529-b141-110fe7c7988d");
  ...
  EXPECT_THAT(
      sutResult.error(),
      Eq(ChunkSettings::Error::REQUIRED_CHUNK_SIZE_EXCEEDS_MAX_CHUNK_SIZE));
}

TEST(ChunkSettings_test,
     CustomUserPayloadAlignmentAndTooLargeUserPayload_Fails) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "fade135d-636f-4b06-8f5d-b33eece1175c");
  constexpr uint32_t USER_PAYLOAD_SIZE{std::numeric_limits<uint32_t>::max()};
...
TEST(ChunkSettings_test, UserHeaderAndTooLargeUserPayload_Fails) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "b61df037-6dae-4350-896e-9ffad96db028");
  constexpr uint32_t USER_PAYLOAD_SIZE{std::numeric_limits<uint32_t>::max()};
  constexpr uint32_t USER_PAYLOAD_ALIGNMENT{alignof(ChunkHeader) * 2};
...
TEST(ChunkSettings_test, UserPayloadAlignmentNotPowerOfTwo_Fails) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "76ad6bad-5175-4475-8834-7a569cb0e489");
  constexpr uint32_t USER_PAYLOAD_SIZE{0U};
  ...
  ASSERT_TRUE(!sutResult.has_value());
  EXPECT_THAT(sutResult.error(),
              Eq(ChunkSettings::Error::ALIGNMENT_NOT_POWER_OF_TWO));
}

TEST(ChunkSettings_test, UserHeaderAlignmentNotPowerOfTwo_Fails) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "2287e0f5-5206-4033-8b23-01fc9ee7d14f");
  constexpr uint32_t USER_PAYLOAD_SIZE{0U};
...
TEST(ChunkSettings_test,
     UserHeaderAlignmentLargerThanChunkHeaderAlignment_Fails) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "6425744a-0ef7-47c3-8819-5c51bb41add7");
  constexpr uint32_t USER_PAYLOAD_SIZE{0U};
  constexpr uint32_t USER_PAYLOAD_ALIGNMENT{
      CHUNK_DEFAULT_USER_PAYLOAD_ALIGNMENT};
  constexpr uint32_t USER_HEADER_SIZE{8U};
...
TEST(ChunkSettings_test, UserHeaderSizeNotMultipleOfAlignment_Fails) {
  ...
  EXPECT_THAT(
      sutResult.error(),
      Eq(ChunkSettings::Error::USER_HEADER_SIZE_NOT_MULTIPLE_OF_ITS_ALIGNMENT));
}
...
class ChunkSettings_AlteringUserPayloadWithoutUserHeader
    : public ::testing::TestWithParam<PayloadParams> {};

// without a user-header, the user-payload is located right after the
// ChunkHeader, therefore the user-payload size and alignment parameters are
// made dependant on the ChunkHeader
INSTANTIATE_TEST_SUITE_P(
    ChunkSettings_test, ChunkSettings_AlteringUserPayloadWithoutUserHeader,
    ::testing::Values(
        // alignment = 0
        ...
        PayloadParams{sizeof(ChunkHeader) * 42U,
                      PayloadParams::MAX_ALIGNMENT}));

TEST_P(ChunkSettings_AlteringUserPayloadWithoutUserHeader,
       RequiredChunkSizeIsCorrect) {
  ::testing::Test::RecordProperty("TEST_ID",
                                  "ec520e0a-043d-43eb-80b5-91e93c89dc62");
  const auto userPayload = GetParam();

  SCOPED_TRACE(
      std::string("User-Payload: size = ") + std::to_string(userPayload.size) +
      std::string("; alignment = ") + std::to_string(userPayload.alignment));
  ...
  const uint32_t expectedSize = [&userPayload] {
    if (userPayload.alignment <= alignof(ChunkHeader)) {
      ...
      auto preUserPayloadAlignmentOverhangOfChunkHeder =
          sizeof(ChunkHeader) - alignof(ChunkHeader);
      return static_cast<uint32_t>(
                 preUserPayloadAlignmentOverhangOfChunkHeder) +
             userPayload.alignment + userPayload.size;
    }
  }();

  auto sutResult =
      ChunkSettings::create(userPayload.size, userPayload.alignment);
  ASSERT_TRUE(sutResult.has_value());
...
class ChunkSettings_AlteringUserPayloadWithUserHeader
    : public ::testing::TestWithParam<PayloadParams> {};
...
INSTANTIATE_TEST_SUITE_P(
    ChunkSettings_test, ChunkSettings_AlteringUserPayloadWithUserHeader,
    ::testing::Values(
        // alignment = 0
        PayloadParams{0U, 0U}, PayloadParams{1U, 0U},
...
uint32_t expectedChunkSizeWithUserHeader(const PayloadParams& userPayload,
                                         uint32_t userHeaderSize) {
  const uint32_t userHeaderSizeAndPaddingToBackOffset = maxVal(
      userHeaderSize, static_cast<uint32_t>(alignof(UserPayloadOffset_t)));

  if (userPayload.alignment <= alignof(UserPayloadOffset_t)) {
    // back-offset is always adjacent to the user-header (as much as possible
    // with the alignment constraints)
    constexpr uint32_t BACK_OFFSET_SIZE{sizeof(UserPayloadOffset_t)};
    return static_cast<uint32_t>(sizeof(ChunkHeader)) +
           userHeaderSizeAndPaddingToBackOffset + BACK_OFFSET_SIZE +
           userPayload.size;
  } else {
    ...
    return static_cast<uint32_t>(sizeof(ChunkHeader)) +
           userHeaderSizeAndPaddingToBackOffset +
...
TEST_P(
  ...
  ::testing::Test::RecordProperty("TEST_ID",
                                  "67b54ff2-0ab6-4fe4-b697-8fca2e22c009");
  const auto userPayload = GetParam();
  ...
  for (const auto userHeaderAlignment : USER_HEADER_ALIGNMENTS) {
    ...
    for (const auto userHeaderSize : USER_HEADER_SIZES) {
      ...
      const uint32_t expectedSize =
          expectedChunkSizeWithUserHeader(userPayload, userHeaderSize);
