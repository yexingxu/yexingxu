...
/// @brief The PublisherPortUser provides the API for accessing a publisher port
/// from the user side. The publisher port is divided in the three parts
/// PublisherPortData, PublisherPortRouDi and PublisherPortUser. The
/// PublisherPortUser uses the functionality of a ChunkSender for sending shared
/// memory chunks. Additionally it provides the offer / stopOffer API which
/// controls whether the publisher port is discoverable for subscriber ports
class PublisherPortUser : public BasePort {
 public:
  ...
  /// PublisherPortUser for being able to cleanup if the user process disappears
  ...
  tl::expected<memory::ChunkHeader*, memory::AllocationError> tryAllocateChunk(
  ...
  void releaseChunk(memory::ChunkHeader* const chunkHeader) noexcept;

  /// @brief Send an allocated chunk to all connected subscriber ports
  /// @param[in] chunkHeader, pointer to the ChunkHeader to send
  void sendChunk(memory::ChunkHeader* const chunkHeader) noexcept;
  ...
  tl::optional<const memory::ChunkHeader*> tryGetPreviousChunk() const noexcept;

  /// @brief offer this publiher port in the system
  void offer() noexcept;

  /// @brief stop offering this publisher port, all subscribers will be removed
  /// from this publisher
  ...
  /// @brief Checks whether there are currently subscribers connected to this
  /// publisher
 ...
 private:
  ...
  memory::ChunkSender<PublisherPortData::ChunkSenderData_t> m_chunkSender;
};
