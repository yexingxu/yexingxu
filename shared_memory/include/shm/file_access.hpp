...
static void outputToStream(StreamType& stream, const char* text,
                           bool& hasPrecedingEntry) {
...
template <typename StreamType>
static void finishEntry(StreamType& stream, const bool hasPrecedingEntry,
                        const bool isLastEntry = false) {
  if (hasPrecedingEntry) {
    stream << "}";
  } else {
    stream << "none}";
...
template <typename StreamType>
static void printOwnerPermissions(StreamType& stream,
                                  const access_rights value) {
...
template <typename StreamType>
static void printGroupPermissions(StreamType& stream,
                                  const access_rights value) {
  bool hasPrecedingEntry = false;
  stream << "group: {";
...
template <typename StreamType>
static void printOtherPermissions(StreamType& stream,
                                  const access_rights value) {
  bool hasPrecedingEntry = false;
  stream << "others: {";
...
template <typename StreamType>
void printAccessControl(StreamType& stream,
