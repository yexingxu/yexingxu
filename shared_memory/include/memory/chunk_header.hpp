...
struct ChunkHeader {
  ...
  /// @param[in] chunkSize is the size of the chunk the ChunkHeader is
  /// constructed
  /// @param[in] chunkSettings are the settings like user-payload size and
  /// user-header alignment
  ChunkHeader(const uint32_t chunkSize,
              const ChunkSettings& chunkSettings) noexcept;

  // copy/move ctors/assignment operators are deleted since the calculations for
  // the user-header and user-payload alignment are dependent on the address of
  // the this pointer
  ...
  /// @brief The ChunkHeader version is used to detect incompatibilities for
  /// record&replay functionality
  /// @return the ChunkHeader version
  uint8_t chunkHeaderVersion() const noexcept;

  /// @brief The id of the user-header used by the chunk; if no user-header is
  /// used, this is set to NO_USER_HEADER
  ...
  /// @brief Get a pointer to the 'ChunkHeader' associated to the user-payload
  /// of the chunk
  /// @param[in] userPayload is the pointer to the user-payload of the chunk
  /// @return the pointer to the 'ChunkHeader' or a 'nullptr' if 'userPayload'
  /// is a 'nullptr'
  static ChunkHeader* fromUserPayload(void* const userPayload) noexcept;

  /// @brief Get a const pointer to the 'ChunkHeader' associated to the
  /// user-payload of the chunk
  /// @param[in] userPayload is the const pointer to the user-payload of the
  /// chunk
  /// @return the const pointer to the 'ChunkHeader' or a 'nullptr' if
  /// 'userPayload' is a 'nullptr'
  static const ChunkHeader* fromUserPayload(
      const void* const userPayload) noexcept;

  /// @brief Get a pointer to the 'ChunkHeader' associated to the user-header of
  /// the chunk
  /// @param[in] userHeader is the pointer to the user-header of the chunk
  /// @return the pointer to the 'ChunkHeader' or a 'nullptr' if 'userHeader' is
  /// a 'nullptr'
  static ChunkHeader* fromUserHeader(void* const userHeader) noexcept;

  /// @brief Get a const pointer to the 'ChunkHeader' associated to the
  /// user-header of the chunk
  ...
  /// @brief Calculates the used size of the chunk with the ChunkHeader,
  /// user-heander and user-payload
  ...
  types::UniquePortId originId() const noexcept;
 ...
 private:
  template <typename T>
  friend class ChunkSender;

  void setOriginId(const types::UniquePortId originId) noexcept;
 ...
 private:
  // the order of these members must be changed carefully and if this happens,
  // the m_chunkHeaderVersion needs to be adapted in order to be able to detect
  // incompatibilities between publisher/subscriber or record&replay,
  // m_chunkSize and m_chunkHeaderVersion should therefore neither changed the
  // type, nor the position

  // size of the whole chunk, including the header
  uint32_t m_chunkSize{0U};
  uint8_t m_chunkHeaderVersion{CHUNK_HEADER_VERSION};
  // reserved for future functionality and used to indicate the padding bytes;
  // currently not used and set to '0'
  uint8_t m_reserved{0};
  // currently just a placeholder
  uint16_t m_userHeaderId{NO_USER_HEADER};
  types::UniquePortId m_originId{types::InvalidPortId};
