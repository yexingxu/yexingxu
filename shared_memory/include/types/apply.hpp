 ...
 * @Version: 2.0
 * @Author: chen, hua
 * @Date: 2024-01-05 13:22:51
 * @LastEditors: chen, hua
 * @LastEditTime: 2024-01-05 13:22:54
 */
///
...
#define DEDUCE_RETURN(EXPR) \
  ->decltype(EXPR) { return EXPR; }

namespace tl {
template <class F, class Tuple, std::size_t... Idx>
auto apply(F&& f, Tuple&& tuple, INDEX_NAMESPACE::index_sequence<Idx...>)
    DEDUCE_RETURN(
        std::forward<F>(f)(std::get<Idx>(std::forward<Tuple>(tuple))...))

        template <class F, class Tuple>
        auto apply(F&& f, Tuple&& tuple) DEDUCE_RETURN(apply(
